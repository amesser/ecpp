...
namespace ecpp
{
  ...
  constexpr uint8_t  ntoh8(uint8_t val)  {return (val);}
  constexpr uint16_t ntoh16(uint16_t val) {return (val >> 8 | val << 8);}
  constexpr uint32_t ntot32(uint32_t val) {return (val >> 24| val << 24 | ((val >> 8) & 0xFF00) | ((val << 8) & 0xFF0000));}
}
